...
namespace renderer {
    ...
    std::vector<svg::Polyline>MapRenderer::GetRouteLines(const std::map<std::string_view, const transport_catalog::Bus*>& buses, const SphereProjector& sp) const {
        std::vector<svg::Polyline> result;
        size_t color_num = 0;
        for (const auto& [bus_number, bus] : buses) {
            std::vector<const transport_catalog::Stop*> route_stops{ bus->stops.begin(), bus->stops.end() };
            if (bus->circle == false) {
    ...
    std::vector<svg::Text> MapRenderer::GetBusLabel(const std::map<std::string_view, const transport_catalog::Bus*>& buses, const SphereProjector& sp) const {
    ...
    std::vector<svg::Circle> MapRenderer::GetStopsMarks(const std::map<std::string_view, const transport_catalog::Stop*>& stops, const SphereProjector& sp) const {
        std::vector<svg::Circle> result;
        for (const auto& [stop_name, stop] : stops) {
            svg::Circle mark;
            mark.SetCenter(sp(stop->coordinates));
            mark.SetRadius(render_settings_.stop_radius);
            mark.SetFillColor("white");
            result.push_back(mark);
        }
        return result;
    }

    std::vector<svg::Text> MapRenderer::GetStopsLabel(const std::map<std::string_view, const transport_catalog::Stop*>& stops, const SphereProjector& sp) const {
        std::vector<svg::Text> result;
        svg::Text text;
        svg::Text underlayer;
    ...
    svg::Document MapRenderer::GetSVG(const std::map<std::string_view, const transport_catalog::Bus*>& buses) const {
        svg::Document result;
        std::vector<geo::Coordinates> route_stops_coord;
        std::map<std::string_view, const transport_catalog::Stop*> all_stops;
        for (const auto& [bus_number, bus] : buses) {
        ...
        for (const auto& circle : GetStopsMarks(all_stops, sp)) {
            result.Add(circle);
        }
        for (const auto& text : GetStopsLabel(all_stops, sp)) {
            result.Add(text);
        }
