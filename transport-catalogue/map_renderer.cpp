...
namespace renderer {
  ...
  std::vector<svg::Polyline>MapRenderer::RouteLines(const std::map<std::string_view, const transport_catalog::Bus*>& buses, const SphereProjector& sp) const {
  ...
  std::vector<svg::Text> MapRenderer::BusLabel(const std::map<std::string_view, const transport_catalog::Bus*>& buses, const SphereProjector& sp) const {
  ...
  std::vector<svg::Circle> MapRenderer::StopsMarks(const std::map<std::string_view, const transport_catalog::Stop*>& stops, const SphereProjector& sp) const {
  ...
  std::vector<svg::Text> MapRenderer::StopsLabel(const std::map<std::string_view, const transport_catalog::Stop*>& stops, const SphereProjector& sp) const {
      std::vector<svg::Text> result;
      svg::Text text;
      svg::Text underlayer;
      for (const auto& [stop_name, stop] : stops) {
  ...
  svg::Document MapRenderer::SVG(const std::map<std::string_view, const transport_catalog::Bus*>& buses) const {
      svg::Document result;
      ...
      for (const auto& circle : StopsMarks(all_stops, sp)) {
          result.Add(circle);
      }
      for (const auto& text : StopsLabel(all_stops, sp)) {
          result.Add(text);
      }
