...
namespace transport_catalog {
	...
	public:
		...
		const std::optional<graph::Router<double>::RouteInfo> FindRoute(const std::string_view stop_from, const std::string_view stop_to);
		const std::vector< graph::Edge<double>>& GetInformation() const;
	...
	private:
		...
		std::vector< graph::Edge<double>> route_info_;
