...
namespace json {
    ...
    void Builder::StartValue(Node node) {
        json::Node* head = nodes_stack_.back();
        if (head->IsMap()) {
            if (!key_) throw std::logic_error("No Key for Dict");
            auto& dict = std::get<Dict>(head->GetValue());
            auto ret = dict.emplace(std::move(key_.value()), node);
    ...
    Node Builder::GetNode(Node value) {
        if (std::holds_alternative<int>(value.GetValue())) {
            return Node(std::get<int>(value.GetValue()));
        }
        if (std::holds_alternative<double>(value.GetValue())) {
            return Node(std::get<double>(value.GetValue()));
        }
        if (std::holds_alternative<std::string>(value.GetValue())) {
            return Node(std::get<std::string>(value.GetValue()));
        }
        if (std::holds_alternative<std::nullptr_t>(value.GetValue())) {
            return Node(std::get<std::nullptr_t>(value.GetValue()));
        }
        if (std::holds_alternative<bool>(value.GetValue())) {
            return Node(std::get<bool>(value.GetValue()));
        }
        if (std::holds_alternative<Dict>(value.GetValue())) {
            return Node(std::get<Dict>(value.GetValue()));
        }
        if (std::holds_alternative<Array>(value.GetValue())) {
            return Node(std::get<Array>(value.GetValue()));
        }
        return {};
    }
